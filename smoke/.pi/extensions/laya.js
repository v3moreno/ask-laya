...
export default function (pi) {
	...
	pi.registerTool({
		name: "laya_triage",
		label: "Laya Triage",
		description: "MANDATORY for classifying documents: returns the message kind of each file. Never classify documents yourself.",
		parameters: { type: "object", required: ["files"], properties: { files: { type: "array", items: { type: "string" } } } },
		async execute(_id, params) {
			layaUsed = true;
			const out = [];
			for (const f of params.files) out.push({ file: f, kind: slim(await predict(readFile(f), TRIAGE_Q)).kind });
			return text(out);
		},
	});
